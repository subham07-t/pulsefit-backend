/*
  ...
  - The `roundTime` column on the `CFRound` table would be dropped and recreated. This will lead to data loss if there is data in the column.
  - The `restTime` column on the `CFRound` table would be dropped and recreated. This will lead to data loss if there is data in the column.
...
-- AlterTable
ALTER TABLE `CFRound` DROP COLUMN `roundTime`,
    ADD COLUMN `roundTime` INTEGER NOT NULL DEFAULT 0,
